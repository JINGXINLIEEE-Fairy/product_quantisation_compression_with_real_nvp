...
from models1.real_nvp.coupling_layer import CouplingLayer, MaskType
...
def real_nvp_model(pretrained=False,device='cuda', **kwargs):
    
    model = RealNVP(num_scales=2, in_channels=3, mid_channels=64, num_blocks=8, **kwargs)
    model = model.to(device) 
    model = torch.nn.DataParallel(model,[0])
    checkpoint = torch.load('99Epoch.pth.tar')
    if pretrained:
