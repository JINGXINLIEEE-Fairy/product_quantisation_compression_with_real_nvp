...
def finetune_centroids(train_loader, student, teacher, criterion, optimizer, epoch=0, n_iter=-1, verbose=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        torch.save(student_logits, 'student_logits_rs.pt')
        torch.save(teacher_probs, 'teacher_probs_rs.pt')
        torch.save(loss, 'loss_rs.pt')
        
        
        print('Student_output_size:', output.size())
    
        print('teacher_output_size:', teacher(input).size())
