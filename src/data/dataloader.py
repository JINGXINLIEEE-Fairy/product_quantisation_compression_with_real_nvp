...
def load_data(data_path='', batch_size=128, nb_workers=64):
    ...
    train_loader = torch.utils.data.DataLoader(
            train_set, batch_size=batch_size, shuffle=True, num_workers=nb_workers, pin_memory=False, drop_last=True)

    test_set = datasets.ImageFolder(
            root=test_data_path, transform=transf_test)
    test_loader = torch.utils.data.DataLoader(
            test_set, batch_size=batch_size, shuffle=False, num_workers=nb_workers, pin_memory=False)
